...
function FacetPanel({ facets, onFacetClick, selectedFacets = {} }: FacetPanelProps) {
  ...
  const handleExpand = (facetName: string) => {
    setExpanded(prev => ({
      ...prev,
  ...
  const renderFacetSection = (config: typeof facetConfig[0]) => {
    const facetData = facets[config.key as keyof Facets] || [];
  ...
  const hasFacets = facetConfig.some(config => 
    facets[config.key as keyof Facets]?.length && facets[config.key as keyof Facets]!.length > 0
  );
